...
The variance is not linear. (Assuming all expectations are well defined)
\noindent\begin{align*}
...
\noindent\begin{align*}
    \mathrm{Var}(a_1X_1 & +\cdots+a_n X_n)  =\sum_{i=1}^{n}a_{i}^{2}\mathrm{Var}(X_{i})+\ldots \\
                        & +2\sum_{i<j}a_{i}a_{j}\mathrm{Cov}(X_{i},X_{j})                      \\
...
\noindent\begin{align*}
    ...
    \operatorname{Cov}(X,Y)  & =\mathbb{E}[XY]-\mathbb{E}[X]\mathbb{E}[Y]=0 \Rightarrow X,Y\text{ indp.} \\
