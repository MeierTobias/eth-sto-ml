...
The likelihood is the probabilty of observing $X$ given that $f$ is parametrized by $\theta$:
...
By the law of large numbers if $X\sim \mathcal{N}(\theta_0, \sigma^2)$, $\theta^*_{MLE}\to\theta_0$ as $n\to\infty$
...
\subsubsection{MAP}
The \textit{maximum a priori} estimator $\theta^*_{MAP}$ is similar to the MLE but with an added prior guess of an underlying distribution of $\theta$, according to \textit{Bayes' law}:
\noindent\begin{align*}
